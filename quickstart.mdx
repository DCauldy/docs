...
```bash cURL
curl -X GET \
  "https://zkqcqgjbsigjdwzmnwmp.supabase.co/functions/v1/v1/users/me" \
  -H "Authorization: Bearer YOUR_API_KEY"
```

```javascript JavaScript
const API_KEY = 'north_sk_live_YOUR_KEY';
const BASE_URL = 'https://zkqcqgjbsigjdwzmnwmp.supabase.co/functions/v1';
...
BASE_URL = 'https://zkqcqgjbsigjdwzmnwmp.supabase.co/functions/v1'
...
```bash cURL
curl -X GET \
  "https://zkqcqgjbsigjdwzmnwmp.supabase.co/functions/v1/v1/reports" \
  -H "Authorization: Bearer YOUR_API_KEY"
```

```javascript JavaScript
const response = await fetch(`${BASE_URL}/v1/reports?limit=10`, {
  headers: {
    'Authorization': `Bearer ${API_KEY}`
  }
});
...
```bash cURL
curl -X PATCH \
  "https://zkqcqgjbsigjdwzmnwmp.supabase.co/functions/v1/v1/reports/REPORT_ID" \
  -H "Authorization: Bearer YOUR_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
...
const response = await fetch(`${BASE_URL}/v1/reports/${reportId}`, {
  method: 'PATCH',
  headers: {
    'Authorization': `Bearer ${API_KEY}`,
    'Content-Type': 'application/json'
...
response = requests.patch(
    f'{BASE_URL}/v1/reports/{report_id}',
    json=updates,
    headers={'Authorization': f'Bearer {API_KEY}'}
)
